...
@section('page-content')
    ...
    @endif
    <form method="POST" action="{{ route('houses.repairs.reply.update', $repairReply->id) }}">
        @csrf
        @method('PATCH')
                                ...
                                <div class="content-wrapper">
                                    ...
                                    <div class="container-xxl flex-grow-1 container-p-y">
                                        ...
                                        <input type="hidden" name="house_id"
                                               value="{{$house_id}}">
                                        <input type="hidden" name="repairReply_id"
                                               value="{{$repairReply->id}}">
                                                    ...
                                                    <div class="mb-3 row">
                                                        <div class="col-md-12">
                                                            <textarea class="form-control" name="contents"
                                                                      aria-label="With textarea" placeholder="請輸入內容"
                                                                      style="height: 248px;">{{$repairReply->content}}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <button type="submit" class="btn btn-info mx-3">更新</button>
